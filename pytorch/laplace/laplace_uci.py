...
def evaluate(model, x_test, y_test, train_mean, train_sd, laplace=False, x_train_normalised=None, subsampling=None, validation=None, optimizer=None):
    
    # evaluate the model on the test/validation set
    if validation == True: # the validation set was normalise, so need to unnormalise it when evaluating
...
def train(model, train_x, train_y, eval_x, eval_y, train_mean, train_sd, validation=False, minibatch_size=None, no_epochs=None, subsampling=None, optimizer=None): # if validation is true, expect eval_x and eval_y to be normalised as well
    ...
    with trange(no_epochs) as epochs:
        for epoch in epochs: # loop over epochs
            if epoch == 0: # at initialisation
                pass
                # MAP_MSE, MAP_NLL = evaluate(model, eval_x, eval_y, train_mean, train_sd, validation=validation, optimizer=optimizer) # without laplace
                # lap_MSE, lap_NLL = evaluate(model, eval_x, eval_y, train_mean, train_sd, laplace=True, x_train_normalised=train_x, subsampling=subsampling, validation=validation, optimizer=optimizer) # with laplace
                # print('MAP_RMSE: {}'.format(torch.sqrt(MAP_MSE)))
                # print('MAP_NLL: {}'.format(MAP_NLL))
                # print('lap_MSE: {}'.format(torch.sqrt(lap_MSE)))
                # print('lap_NLL: {}'.format(lap_NLL))
            ...
            if epoch % 100 == 0 or epoch == no_epochs - 1:
                ...
                # MAP_MSE, MAP_LL = evaluate(model, eval_x, eval_y, train_mean, train_sd, validation=validation, optimizer=optimizer) # without laplace
                # lap_MSE, lap_LL = evaluate(model, eval_x, eval_y, train_mean, train_sd, laplace=True, x_train_normalised=train_x, subsampling=subsampling, validation=validation, optimizer=optimizer) # with laplace
                # print('MAP_RMSE: {}'.format(torch.sqrt(MAP_MSE)))
                # print('MAP_LL: {}'.format(MAP_LL))
                # print('lap_MSE: {}'.format(torch.sqrt(lap_MSE)))
                # print('lap_LL: {}'.format(lap_LL))

                print('noise_sd: {}'.format(torch.sqrt(model.get_noise_var(model.noise_var_param)).data))
...
def individual_train(data_location, test, noise_variance, hidden_sizes, omega, activation_function, learned_noise_var, input_dim, noise_param_init, learning_rate, no_epochs, standard_normal_prior):
    # model
    model = MLP(noise_variance, hidden_sizes, omega, activation=activation_function, learned_noise_var=learned_noise_var, input_dim=input_dim, noise_param_init=noise_param_init, standard_normal_prior=standard_normal_prior)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    # Assume that we are on a CUDA machine, then this should print a CUDA device:
...
def individual_tune_train(results_dir, standard_normal_prior, activation_function, hidden_sizes, omega_range, learning_rate_range, minibatch_size, no_epochs_range, input_dim, subsampling, noise_param_init):
    ...
    # create array of values to grid search over
    list_hypers = [omega_range, learning_rate_range, no_epochs_range]
    ...
    for split in range(20): 
        ...
        for i in range(hyperparams.shape[0]):
            ...
            no_epochs = copy_hyperparams[i,2]
            ...
            MAP_MSE, MAP_LL, lap_LL = individual_train(data_location, test, noise_variance, hidden_sizes, omega, activation_function, learned_noise_var, input_dim, noise_param_init, learning_rate, no_epochs, standard_normal_prior)
            
            # save text file with results
            file = open(results_dir + '/results' + str(split) + '.txt','a') 
            file.write('omega, learning_rate, no_epochs: {} \n'.format(hyperparams[i,:]))
            file.write('MAP_MSE: {} \n'.format(MAP_MSE))
            file.write('MAP_LL: {} \n'.format(MAP_LL))
            file.write('lap_LL: {} \n'.format(lap_LL))
            file.close() 
...
if __name__ == "__main__":
    ...
    noise_param_init = -1
    test = False
    directory = './/experiments//energy_yarin//grid_search'
    ...
    all_RMSE = np.zeros(20)
    all_MAPLL = np.zeros(20)
    all_lapLL = np.zeros(20)
